...
const getOrder = async ({ businessId, userId, orderId }: GetOrderParams) => {
  const where = {
    ...(userId && !businessId
      ? { user_id: userId }
...
const updateOrder = async (orderId: string, input: UpdateOrderInput) => {
  return await prisma.$transaction(async (tx) => {
    ...
    for (const item of input.items) {
      if (item.quantity <= 0)
        throw CustomError.badRequest(
          "La cantidad debe ser mayor a 0 en los items",
        );
    }
    ...
    const itemsToDelete = existingOrder.items.filter(
      (ei) => !orderItemsData.some((oi) => oi.product_id === ei.product_id),
    );
